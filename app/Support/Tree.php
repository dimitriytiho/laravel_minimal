...
class Tree
{
     ...
     *
     * @param array $arr - принимает массив, где id элементов ключи массива.
     * @param string $viewName - название вида из папки resources/views/tree. В вид передаются: объект элемента ($item), элемент вложенности ($tab), id элемента ($id), порядковый номер из цикла ($i).
     * @param string $tab - показывает вложенность, например передать -.
     * @param mixed $values - передать данные для вида, если необходимо, необязательный параметр.
     * @param string $cacheName - по-умолчанию не кэшируется, если надо кэшировать, то передать название кэша, необязательный параметр.
     * @param string $column - колонка связующая, по-умолчанию parent_id, необязательный параметр.
     */
     ...
     *
     * @param string $viewName - название вида из папки resources/views/tree.
     * В вид передаются:
     * объект элемента ($item),
     * элемент вложенности ($tab),
     * id элемента ($id),
     * порядковый номер из цикла ($i).
     * @param array $tree - массив в виде дерева, его строит метод выше get().
     * @param string $tab - показывает вложенность, например передать -.
     * @param mixed $values - передать данные для вида, если необходимо, необязательный параметр.
     * @param string $cacheName - по-умолчанию не кэшируется, если надо кэшировать, то передать название кэша, необязательный параметр.
     */
