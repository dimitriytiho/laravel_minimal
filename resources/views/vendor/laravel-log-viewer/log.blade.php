        ...
        <div class="container-fluid log-viewer">
            <div class="row">
                    ...
                    <div class="list-group list-group-horizontal mb-4 div-scroll">
                                    ...
                                    @if($current_folder == $folder)
                                        <div class="list-group folder">
                                            @foreach($folder_files as $file)
                                                <a href="?l={{ Crypt::encrypt($file) }}&f={{ Crypt::encrypt($folder) }}" class="btn list-group-item @if($current_file == $file) llv-active @endif">{{ $file }}</a>
                                            @endforeach
                                        </div>
                                    @endif
                                </div>
                        ...
                        @if($files)
                            @foreach($files as $file)
                                <a href="?l={{ Crypt::encrypt($file) }}" class="btn list-group-item @if($current_file == $file) llv-active @endif">{{ $file }}</a>
                            @endforeach
                        @endif
                    </div>
                </div>
                <div class="col-12 table-container pr-0 pl-0">
                    ...
                    <div class="p-3">
                        @if($current_file)
                            <a href="?dl={{ Crypt::encrypt($current_file) }}{{ ($current_folder) ? '&f=' . Crypt::encrypt($current_folder) : '' }}" class="mr-3">
                                <span class="fa fa-download"></span>
                                <span>{{ __('log-viewer.download_file') }}</span>
                            </a>
                            <a id="clean-log"
                               href="?clean={{ Crypt::encrypt($current_file) }}{{ ($current_folder) ? '&f=' . Crypt::encrypt($current_folder) : '' }}" class="mr-3">
                                <span class="fa fa-sync"></span>
                                <span>{{ __('log-viewer.clean_file') }}</span>
                            </a>
                            <a id="delete-log"
                               href="?del={{ Crypt::encrypt($current_file) }}{{ ($current_folder) ? '&f=' . Crypt::encrypt($current_folder) : '' }}" class="mr-3">
